...
class RenderTableSelectable extends Component {
	...
	toggleCheck = (c, id) => {
		...
		if (!c) items.splice(items.indexOf(id), 1);
		else items.push(id);
		this.setState({
			itemsChecked: items
		});
		...
		return (
			<div className="rendered-table">
				<div
					className={
						"table-title" +
						(this.state.itemsChecked.length > 0 ? " active" : "")
					}
				>
					...
					<TableHead>
						<TableRow>
							<TableCell padding="checkbox">
								<CheckBox
									checked={
										this.state.itemsChecked.length ===
							...
							.map((element, index) => {
								return (
									<TableRow
										key={"row-" + index}
										selected={
											this.state.itemsChecked.indexOf(
												element[this.props.selectionId]
											) !== -1
										}
									>
										<TableCell padding="checkbox">
